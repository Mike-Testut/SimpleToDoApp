...
def retrieve_items(filepath):
    with open(filepath, "r") as file:
        return file.readlines()

def write_items(filepath,action,item_to_add):
...
while True:
    ...
    if "add" in user_input:
        write_items("to_do_items.txt","a",user_input[4:]+"\n")
        print(f"Successfully added '{user_input[4:]}'")
    elif user_input == "show":
        to_do_list = retrieve_items("to_do_items.txt")
        if not to_do_list:
        ...
        if item_to_delete.isdigit() and 0 < int(item_to_delete) <= len(to_do_list):
            ...
            write_items("to_do_items.txt","w", to_do_list)
